...
def mask_written_value(memory, address, value, mask):
    value_bits = list(reversed(bin(value)[2:])) + ['0'] * len(mask)
    for i, mask_bit in enumerate(reversed(mask)):
        if mask_bit == '0':
            value_bits[i] = '0'
        elif mask_bit == '1':
            value_bits[i] = '1'
    masked_value = int(''.join(reversed(value_bits)), 2)
    memory[address] = masked_value


def mask_address(memory, address, value, mask):
    address_bits = list(reversed(bin(address)[2:])) + ['0'] * len(mask)
    for i, mask_bit in enumerate(reversed(mask)):
        if mask_bit == '1':
            address_bits[i] = '1'
        if mask_bit == 'X':
    ...
    for option in product('01', repeat=address_bits.count('X')):
        ...
        concrete_address = int(''.join(reversed(concrete_address_bits)), 2)
